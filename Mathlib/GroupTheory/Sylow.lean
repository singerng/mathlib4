...
theorem smul_eq_of_normal {g : G} {P : Sylow p G} [h : (P : Subgroup G).Normal] :
...
private theorem not_dvd_index_aux [hp : Fact p.Prime] (P : Sylow p G) [(P : Subgroup G).Normal]
...
/-- A Sylow p-subgroup ahs index indivisible by `p`. -/
theorem not_dvd_index [Fact p.Prime] [Finite (Sylow p G)] (P : Sylow p G)
